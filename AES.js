class AES {
  ...
  keyExpansion(key) {
    ...
    for (let i = 4; i < 44; i++) {
      ...
      if (i % 4 === 0) {
        ...
        // Apply S-box
        temp = temp.map((byte) => AES.Sbox[byte])
        // XOR with Rcon
        temp[0] ^= AES.Rcon[i / 4][0]
      }

      expandedKey[i] = new Array(4)
      for (let j = 0; j < 4; j++) {
  ...
  mixColumns(state) {
    ...
    for (let i = 0; i < 4; i++) {
      const a = state.map((row) => row[i])
      state[0][i] = multiply(2, a[0]) ^ multiply(3, a[1]) ^ a[2] ^ a[3]
      state[1][i] = a[0] ^ multiply(2, a[1]) ^ multiply(3, a[2]) ^ a[3]
      state[2][i] = a[0] ^ a[1] ^ multiply(2, a[2]) ^ multiply(3, a[3])
      state[3][i] = multiply(3, a[0]) ^ a[1] ^ a[2] ^ multiply(2, a[3])
    }
    return state
  }
